...
{
	/* Fix Bug */
    if(getDvar("g_gametype") == "ss" || getDvar("g_gametype") == "dom" || getDvar("g_gametype") == "arcade" || getDvar("g_gametype") == "koth")
    ...
    maps\mp\gametypes\Upgrade::main();
    ...
    level.zombieDeath[9] = "Good Job Team!!!";
    level.zombieDeath[10] = "Humans Win Bitches!";
    level.zombieDeath[11] = "Zombies Death!!!";
...
Shaders()
{
    ...
    precacheShader("cardicon_eagle");
    precacheShader("cardicon_award_jets");
    precacheShader("compassping_enemyfiring");
    precacheShader("cardicon_expert_aug");
    precacheShader("cardicon_expert_fn2000");
    precacheShader("cardicon_expert_kriss");
    precacheShader("cardicon_expert_m21");
    //Gun Icons//
    ...
    precacheModel("viewmodel_sawed_off_double_barrel");
    precacheModel("viewmodel_glock");
...
EndMatch()
{
    ...
    while( 1 )
    {
        players = level.teamCount["spectator"];
        mostKills = getPlayerWithMostKills();
        if(players == level.totalPlayers)
        {
            if(getdvar("z_endgame") == "1" && level.zState != "intermission")
            {
                level notify("humans_all_dead");
                ...
                foreach(player in level.players)
                {
                    foreach( perk in player.perkhud )
                        perk destroy();
                    MusicPlay( "mp_killstreak_carepackage", true ); 
...
onPlayerSpawned()
{
    ...
    for(;;)
    {
        ...
        if(level.zState != "intermission" && self.playerIsDead != 1 && level.mode_doa != 1)
        {
            self.playerIsDead = 1;
        }
        else 
...
onPlayerKilled()
{
	...
	for(;;)
	{
		...
		self thread AImod\BoxFunctions::TakePerks();
