...
MuliKillTextPopup( text, color, glow )
{
	...
	self.multikilltext setPoint("BOTTOMCENTER", "BOTTOMCENTER", 0, -95);
...
BonusDropIcon2(shader, color)
{
    self endon( "disconnect" );
	wait ( 0.05 );
	self.bonusdropicon2 destroy();
	self notify( "bonus_drop_icon" );
	self endon( "bonus_drop_icon" );
	self.bonusdropicon2 = NewClientHudElem( self );
	self.bonusdropicon2 setPoint("BOTTOMCENTER", "BOTTOMCENTER", 0, 0);
	self.bonusdropicon2.color = color;
	self.bonusdropicon2.foreground = true;
	self.bonusdropicon2 setIconShader( shader );
	self.bonusdropicon2 setIconSize( 30, 30 );
	self.bonusdropicon2.alpha = 1;
} //testing thread
...
DropIcon(shader)
{
	...
	self endon("death");
	{
		self.DropIcon setPoint("BOTTOMCENTER", "BOTTOMCENTER", 0, 0);
		self.DropIcon setShader(shader, 10, 10);
		wait 30;
		self.DropIcon.alpha = 0;
	}
} //testing thread
...
GamblerText(text, color, glowcolor, intensity, glowIntensity)
{
	self.gamblertext destroy();
	self.gamblertext = self createFontString("hudbig", 2);
	self.gamblertext setPoint("CENTERMIDDLE", "CENTERMIDDLE", 0, 0);
	self.gamblertext.color = color;
	self.gamblertext.glowcolor = glowcolor;
	self.gamblertext setText(text);
	self.gamblertext.alpha = 0;
	self.gamblertext.glowalpha = glowIntensity;
	self.gamblertext changeFontScaleOverTime(0.25);
	self.gamblertext FadeOverTime(0.25);
	self.gamblertext.fontscale = 0.750;
	self.gamblertext.alpha = intensity;
	wait 0.25;
	self playLocalSound("mp_bonus_end");
	wait 2.75;
	self.gamblertext changeFontScaleOverTime(0.25);
	self.gamblertext FadeOverTime(0.25);
	self.gamblertext.fontscale = 2;
	self.gamblertext.alpha = 0;
	wait 1;
	self.gamblertext destroy();
}