...
ShowHost(player)
{
	...
	for(;;)
	{
		...
		hostname setText("^1AI Zombies eXtreme V2.1 Origins");
		hostname.alpha = 1;
...
SavePlayerWeapons()
{
	...
	if(self hasweapon(self.mulekicktakeweapon))
	{
		weaponList = array_remove(weaponList,self.mulekicktakeweapon);
	}
	self.oldWeaponListWeapon = [];
	self.oldWeaponListClipLeft = [];
	self.oldWeaponListClipRight = [];
	self.oldWeaponListStock = [];
	for (i = 0; i<weaponList.size; i++)
	{
		self.oldWeaponListWeapon[i] = weaponList[i];
		self.oldWeaponListClipLeft[i] = self getweaponammoclip(weaponList[i],"left");
		self.oldWeaponListClipRight[i] = self getweaponammoclip(weaponList[i],"right");
		self.oldWeaponListStock[i] = self getweaponammostock(weaponList[i]);
	}	
}
...
LoadPlayerWeapons()
{
	...
	weaponList = self.oldWeaponListWeapon;
	for (i = 0; i < weaponList .size; i++)
	{
		self giveweapon (weaponList[i]);
		self setweaponammoclip (weaponList[i],self.oldWeaponListClipLeft[i],"left");
		self setweaponammoclip (weaponList[i],self.oldWeaponListClipRight[i],"right");
		self setweaponammostock (weaponList[i],self.oldWeaponListStock[i]);
	}
