...
waitAndSpawnClient()
{
	...
	{
		//Intricate - Have to keep everything like this or else it'll glitch out.
		if( level.zState == "intermission" )
		{
			self AImod\functions::ForceRespawnPlayer(); 
			return;
		}
		else
		{
		...
		//Intricate - THIS PORTION IS IMPORTANT ALWAYS LEAVE THIS DO NOT TOUCH.
		if ( !spawnedAsSpectator )
			self thread	respawn_asSpectator( self.origin + (0, 0, 60), self.angles );
		spawnedAsSpectator = true;
	}
...
lastStandRespawnPlayer()
{
	...
	self thread AImod\_OtherFunctions::SetVision(1);		
...
Callback_PlayerConnect()
{
	...
	if( !level.splitscreen && !isDefined( self.pers["score"] ) )
		iPrintLn("^2" + self.name + " has joined the Apocalypse");
