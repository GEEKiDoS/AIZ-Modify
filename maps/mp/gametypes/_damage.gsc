...
Callback_PlayerDamage_internal( eInflictor, eAttacker, victim, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	if(victim.phd == 1 && sMeansOfDeath != "MOD_MELEE")
			return;
...
{	
	...
	if (self.autorevive >= 1)
		notifyData.titleText = "Quick Revive";
		//notifyData.iconName = "specialty_pistoldeath_upgrade";
	else
		notifyData.titleText = "Last Stand";
		//notifyData.iconName = "specialty_pistoldeath_locked";
	...
	if (self.autorevive >= 1)
	{
		self notify("player_is_down");
		self.lastStandParams = lastStandParams;
		self.inFinalStand = true;	
		self VisionSetNakedForPlayer("mpIntro", 30);
		if(self.tombstone == 1)
		{
				self thread AImod\BoxFunctions::InitTombstone();
		}
		self.tempweapon = self getCurrentWeapon();
		wait 0.5;
		self thread SavePlayerWeapons();
		wait 0.5;
		self takeallweapons();
...
lastStandTimer( delay, isFinalStand )
{
	...
	reviveIcon setShader( "specialty_pistoldeath", 1, 1 );
	reviveIcon setWaypoint( true, true );
	reviveIcon SetTargetEnt( self );
	reviveIcon FadeOverTime( delay ); //we want the transition now!
	reviveIcon.color = (1,0,0); // color red oke
...
reviveTriggerThink( team )
{
	...
	for ( ;; )
	{
		...
		if ( revived )
		{
			...
			self.owner thread LoadPlayerWeapons();
			wait 0.1;
			self.owner switchToWeapon(self.tempweapon);
