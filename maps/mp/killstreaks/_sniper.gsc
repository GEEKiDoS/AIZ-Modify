...
FriendlyBot(gun, text)
{
	self notifyOnPlayerCommand("[{+actionslot 2}]", "+actionslot 2");
	self waittill("[{+actionslot 2}]");	
	self thread AImod\_text::HudRightMessage(text, (1,1,1), (0.3,0.9,0.3), 1, 0.85);
	level thread SpawnChicken(self, gun);
}
...
SpawnChickenFriend(player)
{
	...
	wait 0.2;
	player playLocalSound("exit_prestige");
	wait 1;
